<header x-data="{ mobileMenuOpen: false, userDropdownOpen: false, navDropdownOpen: null }"
        ...
        <div class="flex justify-between h-16">
            ...
            @auth
                    ...
                    @foreach ($menuItems as $item)
                        @if ($item['type'] === 'link')
                            ...
                            <a href="{{ route($item['route']) }}"
                                ...
                                <svg class="w-4 h-4 {{ $isActive ? 'text-emerald-600' : 'text-gray-400' }}" fill="none"
                        ...
                        @elseif($item['type'] === 'dropdown')
                            ...
                            <div class="relative" @click.outside="navDropdownOpen = null">
                                <button
                                    ...
                                    <svg class="w-4 h-4 {{ $isParentActive ? 'text-emerald-600' : 'text-gray-400' }}"
                                    ...
                                    <svg class="w-3 h-3 ml-0.5 transition-transform duration-200"
                                        :class="navDropdownOpen === '{{ $item['id'] }}' ? 'rotate-180' : ''"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <!-- Dropdown Panel -->
                                <div x-show="navDropdownOpen === '{{ $item['id'] }}'"
                                    ...
                                    @foreach ($item['children'] as $child)
                                        ...
                                        <a href="{{ route($child['route']) }}"
                                            class="block px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors {{ $isChildActive ? 'text-emerald-600 font-medium bg-emerald-50/50' : 'text-gray-700' }}">
                                            {{ $child['label'] }}
                                        </a>
                                    @endforeach
                                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <!-- Notification Bell -->
                    <button
                        class="p-2 rounded-full text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 transition hidden sm:block relative">
                    ...
                    <div class="relative ml-2">
                        <button @click="userDropdownOpen = !userDropdownOpen" @click.away="userDropdownOpen = false"
                            class="flex items-center gap-2 focus:outline-none p-1 rounded-full hover:bg-gray-50 transition">
                            <div
                                class="h-9 w-9 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-700 font-bold text-sm">
                            ...
                            <div class="hidden md:flex flex-col items-start pr-2">
                                ...
                                <span
                                    class="text-[10px] text-gray-500 uppercase tracking-wide">{{ Auth::user()->role }}</span>
                            </div>
                        ...
                        <div x-show="userDropdownOpen" x-transition:enter="transition ease-out duration-100"
                            ...
                            <a href="{{ route('profile') }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-emerald-600 transition-colors">Mon
                                Profil</a>
                            @if ($isAdmin)
                                <a href="{{ route('admin.settings') }}"
                                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-emerald-600 transition-colors">Paramètres</a>
                            @endif
                ...
                @foreach ($menuItems as $item)
                    @if ($item['type'] === 'link')
                        ...
                        <a href="{{ route($item['route']) }}"
                            ...
                            <svg class="w-5 h-5 {{ $isActive ? 'text-emerald-600' : 'text-gray-400' }}" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="{{ $item['icon'] }}"></path>
                            </svg>
                    ...
                    @elseif($item['type'] === 'dropdown')
                        ...
                        <div x-data="{ expanded: false }">
                            <button @click="expanded = !expanded"
                                class="w-full flex justify-between items-center px-3 py-2 rounded-md text-base font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900">
                            ...
                            <div x-show="expanded" x-collapse class="pl-11 space-y-1 mt-1">
                                @foreach ($item['children'] as $child)
                                    @php $isChildActive = Route::currentRouteName() === $child['route']; @endphp
                                    <a href="{{ route($child['route']) }}"
                                        class="block py-2 text-sm rounded-md hover:text-emerald-600 {{ $isChildActive ? 'text-emerald-700 font-bold' : 'text-gray-500' }}">
                                        {{ $child['label'] }}
                                    </a>
                                @endforeach
                            </div>
