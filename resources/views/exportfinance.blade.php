...
<head>
    ...
    <title>Ressources Financières</title>
    @php
...
<body>
    <div class="header">
        ...
        @endif
        <div>
            <div class="title">Ressources Financières</div>
    ...
    <table>
        ...
        <tbody>
            @foreach($rfs as $rf)
            <tr>
                <td>{{ $rf->centre->denomination ?? '—' }}</td>
                <td>{{ $rf->budget_annee }}</td>
                <td>{{ number_format($rf->total_recettes, 2, ',', ' ') }} DH</td>
                <td>{{ number_format($rf->total_depenses, 2, ',', ' ') }} DH</td>
                <td>{{ number_format($rf->total_recettes - $rf->total_depenses, 2, ',', ' ') }} DH</td>
            </tr>
            @endforeach
        </tbody>
    </table>
